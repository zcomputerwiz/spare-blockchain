...
# sets the version for spare-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$SPARE_INSTALLER_VERSION" '.version=$VER' package.json > temp.json && mv temp.json package.json

electron-packager . spare-blockchain --asar.unpack="**/daemon/**" --platform=linux \
--icon=src/assets/img/Spare.icns --overwrite --app-bundle-id=net.spare.blockchain \
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create spare-blockchain-$SPARE_INSTALLER_VERSION.rpm"

	# shellcheck disable=SC2046
	NODE_ROOT="$(dirname $(dirname $(which node)))"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Spare-Network/spare-blockchain/issues/3846
  ...
  --arch "$REDHAT_PLATFORM" --options.version $SPARE_INSTALLER_VERSION \
  --license ../LICENSE
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
